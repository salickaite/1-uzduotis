...
int main()
{
    ...
    {
        cout << "Netinkama ivestis. Iveskite F (failo) arba R (ranka). Bandykite dar karta: ";
        cin >> read_type;
    }
        ...
        while (open_f)
        {
            if (!open_f.eof())
            {
                ...
                for (int j = 0; j < cols - 3; j++)
                {
                    ...
                    try
                    {
                        if (temp_paz < 1 || temp_paz > 10) throw std::runtime_error("Namu darbu pazymiai turi buti nuo 1 iki 10. Patikrinkite faile esancius duomenis. ");
                    }
    ...
    if (read_type == "R" || read_type == "r")
    {
        ...
        while (ivestis != "A" && ivestis != "a" && ivestis != "P" && ivestis != "p")
        {
            try
            {
                if (ivestis != "A" && ivestis != "a" && ivestis != "P" && ivestis != "p") throw std::invalid_argument("Neatpazistama komanda. ");
            }
            ...
            if (ivestis == "P" || ivestis == "p")
            {
                ...
                while (cin >> laik_paz)
                {
                    while (laik_paz < 1 || laik_paz > 10) {
                        try
                        {
                            if (laik_paz < 1 || laik_paz > 10) throw std::invalid_argument("Namu darbu pazymiai turi buti nuo 1 iki 10. ");
                        }
                        catch (std::invalid_argument& e)
                        {
                            cout << e.what() << endl;
                ...
                while (temp_student.egz < 1 || temp_student.egz > 10)
                {
                    try
                    {
                        if (temp_student.egz < 1 || temp_student.egz > 10) throw std::invalid_argument("Egzamino pazymys turi buti nuo 1 iki 10. ");
                    }
                    catch (std::invalid_argument& e)
                    {
                        cout << e.what() << endl;
